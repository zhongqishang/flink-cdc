...
public class RecordDataWrapper implements StructLike {
    ...
    public RecordDataWrapper(RowType rowType, Types.StructType struct) {
    ...
    public RecordDataWrapper wrap(RecordData data) {
    ...
    public <T> T get(int pos, Class<T> javaClass) {
        if (recordData.isNullAt(pos)) {
            ...
            return javaClass.cast(getters[pos].get(recordData, pos));
        }

        Object value =
                RecordData.createFieldGetter(types[pos], pos).getFieldOrNull(this.recordData);
    ...
    private static PositionalGetter<?> buildGetter(LogicalType dataType, Type type) {
        switch (dataType.getTypeRoot()) {
            ...
            case ROW:
                ...
                RecordDataWrapper nestedWrapper = new RecordDataWrapper(rowType, structType);
