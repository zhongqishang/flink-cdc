...
/** Copy from iceberg IcebergFilesCommitter. */
class IcebergTableFilesCommitter extends AbstractStreamOperator<Void>
    ...
    // than
    // the max committed one to iceberg table, for avoiding committing the same data files twice.
    // This
    ...
    // A sorted map to maintain the completed data files for each pending checkpointId (which have
    // not
    ...
    // interrupted because of network/disk failure etc, while we don't expect any data loss in
    // iceberg
    // table. So we keep the finished files <1, <file0, file1>> in memory and retry to commit
    // iceberg
    ...
    // another different job. For the second case, we need to maintain the old flink job's id in
    // flink
    ...
    public void initializeState(StateInitializationContext context) throws Exception {
        ...
        if (context.isRestored()) {
            ...
            // Since flink's checkpoint id will start from the max-committed-checkpoint-id + 1 in
            ...
            if (!uncommittedDataFiles.isEmpty()) {
                NavigableMap<Long, byte[]> uncommittedSchemaChangeDataFiles =
                        Maps.newTreeMap(checkpointSortedMap).headMap(0L, false);
    ...
    public void notifyCheckpointComplete(long checkpointId) throws Exception {
        ...
        // For step#4, we don't need to commit iceberg table again because in step#3 we've committed
        // all the files, Besides, we need to maintain the max-committed-checkpoint-id to be
        // increasing.
    ...
    private void commitPendingResult(
        ...
        if (totalFiles != 0 || continuousEmptyCheckpoints % maxContinuousEmptyCommits == 0) {
            ...
            LOG.info(
                    "Skip commit for checkpoint {} due to no data files or delete files.",
                    checkpointId);
        }
    }
    ...
    private void commitDeltaTxn(
        ...
        if (summary.deleteFilesCount() == 0) {
            ...
            commitOperation(
                    appendFiles, summary, "append", newFlinkJobId, operatorId, checkpointId);
        } else {
            // To be compatible with iceberg format V2.
            for (Map.Entry<Long, WriteResult> e : pendingResults.entrySet()) {
                // We don't commit the merged result into a single transaction because for the
                // sequential transaction txn1 and txn2, the equality-delete files of txn2 are
                // required to be applied to data files from txn1. Committing the merged one will
                // lead to the incorrect delete semantic.
                ...
                // may push deletes further in the future, but do not affect correctness. Position
                // deletes committed to the table in this path are used only to delete rows from
                // data files that are being added in this commit. There is no way for data files
                // added along with the delete
                // files to be concurrently removed, so there is no need to validate the files
                // referenced by the position delete files that are being committed.
                RowDelta rowDelta = table.newRowDelta().scanManifestsWith(workerPool);

                Arrays.stream(result.dataFiles()).forEach(rowDelta::addRows);
                Arrays.stream(result.deleteFiles()).forEach(rowDelta::addDeletes);
                commitOperation(
                        rowDelta, summary, "rowDelta", newFlinkJobId, operatorId, e.getKey());
            }
        }
    }
    ...
    private void commitOperation(
        ...
        // custom snapshot metadata properties will be overridden if they conflict with internal
        // ones
        // used by the sink.
    ...
    public void endInput() throws IOException {
        ...
        commitUpToCheckpoint(
                dataFilesPerCheckpoint, flinkJobId, operatorUniqueId, currentCheckpointId);
    }
