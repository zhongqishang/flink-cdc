...
public class EventWrapper implements StructLike {
    private final LogicalType[] types;
    private final EventWrapper.PositionalGetter<?>[] getters;
    private RowData rowData = null;
    private DataChangeEvent event = null;

    public EventWrapper(RowType rowType, Types.StructType struct) {
        int size = rowType.getFieldCount();

        types = (LogicalType[]) Array.newInstance(LogicalType.class, size);
        getters =
                (EventWrapper.PositionalGetter[])
                        Array.newInstance(EventWrapper.PositionalGetter.class, size);
    ...
    public EventWrapper wrap(DataChangeEvent event) {
        this.event = event;
        return this;
    }
    ...
    private static EventWrapper.PositionalGetter<?> buildGetter(
